...
class BaseClass:
    def __repr__(self) -> str:
        ...
        attributes = ", ".join(f"{k}={type(v)}" for k, v in self.__dict__.items())
        return f"{class_name}({attributes})"
        
    def __str__(self) -> str:
        """Provide a summary of the current class status.

        :return: Short description of the class state with attributes and values.
        :rtype: str
        """
        ...
        cls_status += "-" * (len(class_name) + 39) + "\n\n"
        # Loop over the class attributes to add the respective info state to the str
        for name in vars:
            attr = getattr(self, name)
            if name == 'labels':
                attr = list(attr)
            if isinstance(attr, np.ndarray):
                cls_status += f"  \u2022 {name}, array of shape -> {attr.shape}\n"
